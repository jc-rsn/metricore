...
__all__ = ('Snowflake', 'SnowflakeGenerator', 'MAX_TIMESTAMP', 'MAX_INSTANCE', 'MAX_SEQUENCE')

MAX_TIMESTAMP = 0b11111111111111111111111111111111111111111
MAX_INSTANCE = 0b1111111111
MAX_SEQUENCE = 0b111111111111
BASE_EPOCH = datetime.utcfromtimestamp(0)
...
class Snowflake:
    ...
    def __post_init__(self):
        ...
        if self.timestamp < 0 or self.timestamp > MAX_TIMESTAMP:
            raise ValueError(f"timestamp must not be negative and must be less than {MAX_TIMESTAMP}!")
        ...
        if self.seq < 0 or self.seq > MAX_SEQUENCE:
            raise ValueError(f"seq must not be negative and must be less than {MAX_SEQUENCE}!")
...
class SnowflakeGenerator:
    def __init__(self, instance: int, *, seq: int = 0, epoch: int = 0, timestamp: Optional[int] = None):
        ...
        if current - epoch >= MAX_TIMESTAMP:
